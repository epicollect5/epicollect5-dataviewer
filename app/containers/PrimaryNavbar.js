...
class PrimaryNavbar extends React.Component {
  ...
  render() {
    ...
    const v = Math.floor(new Date(projectStats.structure_last_updated).getTime() / 1000);
    const logoUrl = PARAMETERS.SERVER_URL + PARAMETERS.API_MEDIA_ENDPOINT + projectSlug + PARAMETERS.PROJECT_LOGO_QUERY_STRING + '&v=' + v;
      ...
      return (
        <Navbar fixedTop fluid className="view-children-navbar animated fadeIn">
          ...
          <Navbar.Text pullRight>
            ...
            <span
              className="label__current-parent-title hidden-xs hidden-sm"> of "{helpers.textTruncate(parentTitle)}" </span>
          </Navbar.Text>
        </Navbar>
      );
    }
